...
export const FullPageLoader = ({ label = "Loading..." }) => {
  return (
    <div className="flex min-h-screen items-center justify-center bg-slate-50 px-4">
      <div className="flex items-center gap-3 rounded-lg border border-slate-200 bg-white px-6 py-4 text-slate-600 shadow-sm">
...
export const SectionLoader = ({ label = "Loading data..." }) => {
  return (
    <div className="flex items-center justify-center gap-3 rounded-xl border border-dashed border-slate-300 bg-white/70 px-6 py-10 text-slate-600">
      <LoaderCircle className="h-5 w-5 animate-spin text-primary" />
      <span>{label}</span>
    </div>
  );
